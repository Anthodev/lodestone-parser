...
class ParseCharacter extends ParseAbstract implements Parser
{
    ...
    private const TX_RACECLANGENDER = ['Race/Clan/Gender', 'Volk / Stamm / Geschlecht', 'Race / Ethnie / Sexe', '種族/部族/性別'];
    private const TX_NAMEDAY        = ['Nameday', 'Guardian', 'Namenstag', 'Schutzgott', 'Date de naissance', 'Divinité', '誕生日', '守護神'];
    private const TX_TOWN           = ['City-state', 'Stadtstaat', 'Cité de départ', '開始都市'];
    private const TX_GRANDCOMPANY   = ['Grand Company', 'Staatliche Gesellschaft', 'Grande compagnie', '所属グランドカンパニー'];
     ...
     */
    public function handle(string $htmlContent): CharacterProfile
    {
        // set dom
    ...
    private function parseProfile(): void
    {
        ...
        foreach ($blocks as $block) {
            ...
            if (in_array($blockTitle, self::TX_RACECLANGENDER, true)) {
                $this->parseProfileRaceTribeGender($block);
            } elseif (in_array($blockTitle, self::TX_NAMEDAY, true)) {
                $this->parseProfileNameDay($block);
            } elseif (in_array($blockTitle, self::TX_TOWN, true)) {
                $this->parseProfileTown($block);
            } elseif (in_array($blockTitle, self::TX_GRANDCOMPANY, true)) {
    ...
    private function parseAttributes(): void
    {
        ...
        foreach ($this->dom->find('.character__param__list tr') as $tr) {
            ...
            $attribute->Name  = $tr->find('th')->text();
            $attribute->Value = $tr->find('td')->text();

            $this->profile->GearSet['Attributes'][] = $attribute;
        }
        ...
        foreach ($this->dom->find('.character__param ul li') as $li) {
            $attribute        = new Attribute();
            $attribute->Name  = $li->find('p')->text();
            $attribute->Value = $li->find('span')->text();
     ...
     */
    private function parseEquipGear(): void
    {
        /**
         * @var int $i
         * @var DomQuery $node
        ...
        foreach ($this->dom->find('.character__view')->eq(0)->find('.item_detail_box') as $i => $node) {
            ...
            if (
                trim($materiaNodes->html())
                && $materiaNodes = $materiaNodes->find('li')
            ) {
                /** @var DomQuery $materiaNode */
                foreach ($materiaNodes as $materiaNode) {
                    $mhtml = $materiaNode->find('.db-tooltip__materia__txt')->html();
                    if (!$mhtml) {
                        continue;
                    }

                    $materiaDetails = explode('<br>', html_entity_decode($mhtml));
                    if (empty($materiaDetails[1])) {$materiaDetails[1] = null;}

                    $materiaObject = new ItemSimple();
                    $materiaObject->Name  = trim(strip_tags($materiaDetails[0]));
                    $materiaObject->Value = trim(strip_tags($materiaDetails[1]));
     ...
     */
    private function parseProfileBasic(): void
    {
        // name
     ...
     */
    private function parseProfileBio(): void
    {
     ...
     * @param DomQuery $node
     */
    private function parseProfileRaceTribeGender($node): void
    {
     ...
     * @param DomQuery $node
     */
    private function parseProfileNameDay($node): void
    {
     ...
     * @param DomQuery $node
     */
    private function parseProfileTown($node): void
    {
        $obj = new Town();
        $obj->Name = html_entity_decode($node->find('.character-block__name')->text(), ENT_QUOTES, "UTF-8");
        $obj->Icon = $node->find('img')->attr('src');
     ...
     * @param DomQuery $node
     */
    private function parseProfileGrandCompany($node): void
    {
        $html = $node->find('.character-block__name')->html();
     ...
     * @param DomQuery $node
     */
    private function parseProfileFreeCompany($node): void
    {
     ...
     * @param DomQuery $node
     */
    private function parseProfilePvPTeam($node): void
    {
